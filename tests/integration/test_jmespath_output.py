...
class JMESPathTests(CliTestCase):
    ...
    def test_jmespath_noop(self):
        ...
        output = self.run_line("globus endpoint show {} -Fjson".format(GO_EP1_ID))
    ...
    def test_jmespath_extract_from_list(self):
        ...
        show_output = self.run_line("globus endpoint show {} -Fjson".format(GO_EP2_ID))
    ...
    def test_jmespath_no_expression_error(self):
        ...
        output = self.run_line(
            "globus endpoint search 'Tutorial' --jq", assert_exit_code=2
        )
