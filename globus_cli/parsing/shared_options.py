...
def task_submission_options(f):
    ...
    def notify_opt_callback(ctx, param, value):
        ...
        if (vals or on) and off:
            raise click.UsageError(
                '--notify cannot accept "off" and another value')

        allowed_vals = set(("on", "succeeded", "failed", "inactive"))
        if not vals <= allowed_vals:
            raise click.UsageError(
                '--notify received at least one invalid value among {}'
                .format(list(vals)))
    ...
    f = click.option(
        ...
        help=("Don't actually submit the task, print submission "
              "data instead"))(f)
    f = click.option(
        "--notify", callback=notify_opt_callback,
        help=("Comma separated list of task events which notify by email. "
